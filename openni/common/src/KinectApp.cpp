...
// Needed for SDL_GetTicks(). Remove this as soon as it was replaced
//  by standard chrono classes.
#include <SDL.h>
...
void KinectApp::InitSdlVideo(const Size& res)
{
   // Bring up SDL's video system...
   if (0 > SDL_Init(SDL_INIT_VIDEO)) {
      throw "Cannot init SDL video subsystem.";
   }
   // ...make sure everything gets cleaned up at exit...
   atexit(SDL_Quit);

   // ...and finally create the window.
   const SDL_Surface* screen = SDL_SetVideoMode(res.Width,
                                                res.Height,
                                                32,
                                                SDL_ANYFORMAT |
                                                   SDL_SWSURFACE |
...
void KinectApp::InitGlVideo(const Size& res)
{
   if (0 > SDL_Init(SDL_INIT_VIDEO)) {
      throw "Cannot init SDL video subsystem.";
   }
